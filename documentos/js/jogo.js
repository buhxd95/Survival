!function(){
    ...
    var canvas = $("#canvas"),
        ...
        andar:() => {
            var parado = true,
				velocidade = jogo.roundAtual>=20 && jogo.jogador.hp>=100+jogo.hpPorRound*jogo.roundAtual*0.8 ? 4 : 2;
            ...
            switch(jogo.jogador.direcao){
                case 1:
                    jogo.jogador.imagem = imagem("norte",parado);
                    break;
                case 2:
                    jogo.jogador.imagem = imagem("leste",parado);
                    break;
                case 3:
                    jogo.jogador.imagem = imagem("sul",parado);
                    break;
                case 4:
                    jogo.jogador.imagem = imagem("oeste",parado);
                    break;
            }
        }
    	},
	...
	function movimento(direcao,obj,velocidade){
	...
	function atirar(obj,width,height,direcao){
		ctx.fillStyle = 'black';
		ctx.fillRect(obj.x, obj.y, width, height);
		movimento(direcao,obj,20);
	}
	...
	function imagem(img,parado,obj,velocidade,direcao){
		if(img.indexOf("i_") !== -1){
			movimento(direcao,obj,velocidade);
			obj.direcao = Math.round(jogo.contar/10)%2 ? "documentos/imagens/"+img+"1.png" : "documentos/imagens/"+img+"2.png";
		} else
			return Math.round(jogo.contar/10)%2 || parado ? "documentos/imagens/"+img+"1.png" : "documentos/imagens/"+img+"2.png";
	}
    ...
    function desenhar(){
        ...
        $.each(jogo.balas,function(i){
            if(jogo.balas[i].x < width && jogo.balas[i].direcao.indexOf("leste") != -1)
				atirar(jogo.balas[i], 7, 1, 1);				//velocidade de 20 a cada desenhar
            if(jogo.balas[i].y < height && jogo.balas[i].direcao.indexOf("sul") != -1)
				atirar(jogo.balas[i], 1, 7, 2);
            if(jogo.balas[i].x > 0 && jogo.balas[i].direcao.indexOf("oeste") != -1)//checar se as balas estao dentro do canvas, e atirar de acordo com a posiçao do boneco
   				atirar(jogo.balas[i], 7, 1, 3);						//criar balas com cada indice dentro do objeto
            if(jogo.balas[i].y > 0 && jogo.balas[i].direcao.indexOf("norte") != -1)
                atirar(jogo.balas[i], 1, 7, 4);
        ...
        $.each(jogo.inimigos,function(i){
            ...
            if(jogo.roundAtual<20 && !jogo.gogogo){//se o round é menor que 20, o inimigo anda 1 dimensão por vez (x,y)
                if(jogo.inimigos[i].x < jogo.jogador.x && (jogo.inimigos[i].x !== jogo.jogador.x))
                    imagem("i_leste",null,jogo.inimigos[i],velocidade,1);
                else if(jogo.inimigos[i].y < jogo.jogador.y  && (jogo.inimigos[i].y !== jogo.jogador.y))
                    imagem("i_sul",null,jogo.inimigos[i],velocidade,2);
                else if(jogo.inimigos[i].x > jogo.jogador.x  && (jogo.inimigos[i].x !== jogo.jogador.x))
                    imagem("i_oeste",null,jogo.inimigos[i],velocidade,3);
                else if(jogo.inimigos[i].y > jogo.jogador.y  && (jogo.inimigos[i].y !== jogo.jogador.y))
                    imagem("i_norte",null,jogo.inimigos[i],velocidade,4);
            }
            else if(jogo.roundAtual>=20 || jogo.gogogo){	//se é maior que 20, o zumbi anda na diagonal
                if(jogo.inimigos[i].x < jogo.jogador.x && (jogo.inimigos[i].x !== jogo.jogador.x))
                    imagem("i_leste",null,jogo.inimigos[i],1,1);
                if(jogo.inimigos[i].y < jogo.jogador.y  && (jogo.inimigos[i].y !== jogo.jogador.y))
                    imagem("i_sul",null,jogo.inimigos[i],1,2);
                if(jogo.inimigos[i].x > jogo.jogador.x  && (jogo.inimigos[i].x !== jogo.jogador.x))
                    imagem("i_oeste",null,jogo.inimigos[i],1,3);
                if(jogo.inimigos[i].y > jogo.jogador.y  && (jogo.inimigos[i].y !== jogo.jogador.y))
                    imagem("i_norte",null,jogo.inimigos[i],1,4);
            }
        });
